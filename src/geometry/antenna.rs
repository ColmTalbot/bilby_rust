...
const GEOCENTER: ThreeVector = ThreeVector { x: 0.0, y: 0.0, z: 0.0 };
...
pub fn get_polarization_tensor(
    ...
    let result = std::panic::catch_unwind(
        || polarization_tensor(ra, dec, gps_time, psi, mode).into()
    );
...
fn _time_delay_from_vertices(
    vertex_1: ThreeVector, vertex_2: ThreeVector, ra: f64, dec: f64, gps_time: f64
) -> f64 {
